...
function init()
{
    const elements = document.getElementsByClassName("lorem-text");
    for (let i = 0; i < elements.length; i++)
    {
        elements[ i ].innerHTML = generate_lorem(elements[ i ].getAttribute('length'));
    }
}

function generate_lorem(value)
{
    ...
    tempText = tempText.map(function (item) { return `<p style="text-align: justify">${item}</p>`; }).join("");
